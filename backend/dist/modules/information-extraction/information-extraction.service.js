...
var InformationExtractionService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.InformationExtractionService = void 0;
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("../database/prisma.service");
const openai_1 = require("openai");
const config_1 = require("@nestjs/config");
let InformationExtractionService = InformationExtractionService_1 = class InformationExtractionService {
    constructor(prisma, configService) {
        this.prisma = prisma;
        this.configService = configService;
        this.logger = new common_1.Logger(InformationExtractionService_1.name);
        this.openai = new openai_1.OpenAI({
            apiKey: this.configService.get('OPENAI_API_KEY'),
        });
    }
    async extractAndStoreInformation(callId, businessId, transcript, targetPerson, goal) {
        ...
        try {
            ...
            const extractedInfo = await this.aiExtractInformation(transcript, business, targetPerson, goal);
    ...
    async aiExtractInformation(transcript, business, targetPerson, goal) {
    ...
    buildExtractionPrompt(transcript, business, targetPerson, goal) {
        return `
...
`;
    ...
    async storeExtractedInformation(callInfo) {
        const extractedRecord = await this.prisma.extractedInformation.create({
            data: {
                ...
                contactInfo: callInfo.contactInfo,
                operationalInfo: callInfo.operationalInfo,
                callOutcome: callInfo.callOutcome,
                metadata: callInfo.metadata,
    ...
    async storeInformationEntities(callInfo) {
    ...
    async searchExtractedInformation(query) {
        const whereClause = {};
        if (query.businessId) {
        ...
        return results.map(result => ({
            callInformation: {
                ...
                operationalInfo: result.operationalInfo,
    ...
    determineInformationQuality(confidence) {
        if (confidence >= 0.8)
            return 'high';
        if (confidence >= 0.6)
            return 'medium';
    ...
    determinePersonCategory(person, callInfo) {
        if (callInfo.contactInfo.targetPerson?.name === person) {
            return 'target_person';
        }
        return 'mentioned_person';
    }
    determinePhoneCategory(phone, callInfo) {
    ...
    calculateRelevanceScore(result, query) {
        ...
        if (result.goalAchieved)
            score += 0.2;
        if (result.informationQuality === 'high')
            score += 0.2;
        else if (result.informationQuality === 'medium')
            score += 0.1;
...
exports.InformationExtractionService = InformationExtractionService;
exports.InformationExtractionService = InformationExtractionService = InformationExtractionService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        config_1.ConfigService])
], InformationExtractionService);
//# sourceMappingURL=information-extraction.service.js.map